...
public class ClaseDAO {
    ...
    public boolean ActualizarClase(Clase clase) {
        ...
        try {
            ...
            PreparedStatement pstm = con.prepareStatement(" UPDATE clase  SET  IDCURSOPORGRADO=?, "
                    + "iddocente =?,  observacion =? WHERE id=? ");

            pstm.setInt(1, clase.getCursoGrado().getId());
            pstm.setInt(2, clase.getDocente().getId());
            pstm.setString(3, clase.getObservacion());
            pstm.setInt(4, clase.getId());
    ...
    public boolean EliminarClase(Clase clase) {
        ...
        try {
            Connection con = Conexion.getConnection();
            PreparedStatement pstm = con.prepareStatement(" UPDATE clase  SET  IDCURSOPORGRADO=?,  "
                    + "iddocente =?,  observacion =?, estado = 0 WHERE id=? ");
    ...
    public List<Clase> ListarClases() {
        ...
        try {
            Connection con = Conexion.getConnection();
            PreparedStatement pstm = con.prepareStatement("select g.NUMEROGRADO, s.DESCRIPCION as seccion, n.NOMBRELARGO as nivel, c.NOMBRELARGO as nombreCurso, d.NOMBRE, d.APELLIDOPATERNO, cla.OBSERVACION, \n"
    ...
    public List<Clase> ListarClasesPorGrado(Grado grado) {
        ...
        try {
            Connection con = Conexion.getConnection();
            PreparedStatement pstm = con.prepareStatement("select  cla.id as idClase, c.NOMBRELARGO, d.NOMBRE, d.APELLIDOPATERNO, d.APELLIDOMATERNO\n"
